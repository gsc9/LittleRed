...
public abstract class StdGame extends JGEngine {
	...
	/** ADDED: Invincibility is disabled by default */
	public boolean invincible = false;
	/** ADDED: Game title will be the class's name by default. */
	public String title = "";
	/** ADDED: Key for advancing a level is 'G'. */
	public int key_skip = 'G';
	/** ADDED: Key for infinite lives is 'I'. */
	public int key_invincibility = 'I';
	/** Enemy lives count, 0 means game over. */
	...
	* Can be used to vary graphic sets, display stage number, etc. */
	...
	public JGFont status_font = new JGFont("Cambria",Font.BOLD,10);
	/** Color to use to display score */
	public JGColor status_color = JGColor.white;
	/** Image to use to display lives */
	public String lives_img = "lives";
	/** Image to use to display boss's lives */
	public String boss_lives_img = "boss_lives";
	/** Font to use to display title and messages */
	public JGFont title_font = new JGFont("Cambria",0,16);
	...
	* initAppConfig to initialise it. */
	...
	public void setStatusDisplayEnemy(JGFont status_font, JGColor status_color,
	String boss_lives_img) {
		this.status_font=status_font;
		this.status_color=status_color;
		this.boss_lives_img=boss_lives_img;
	}
	...
	public int highscore_waittime=1000;
	...
	public JGFont highscore_font = new JGFont("Cambria",Font.BOLD,14);
	...
	public JGFont highscore_title_font = new JGFont("Cambria",Font.BOLD,14);
	...
	* When boss_lives_img is set, it uses another image to display boss lives.*/
	public void paintFrame() {
		if(startgame_ingame == true) {
			setFont(status_font);
			setColor(status_color);
			if (lives_img==null) {
				drawString("Your Lives "+lives,viewWidth()-status_r_margin,0,1);
			} else {
				drawCount(lives-1, lives_img, viewWidth()-status_r_margin,0,
						- getImageSize(lives_img).x-2 );
			}
			if(stage > 0) {
				if(boss_lives_img == null) {
					drawString("Boss Lives "+bossLives,status_r_margin,0,1);
				} else {
					drawCount(bossLives-1,boss_lives_img,status_r_margin,0,
							getImageSize(boss_lives_img).x-2);
				}
			}
		}
	}
	...
	public void paintFrameTitle() {
		drawString((title.equals("")) ? getClass().getName().substring(getClass().getName()
			.lastIndexOf('.')+1) : title,
...
//			.lastIndexOf('.' )+1 ),
		...
		char key = getLastKeyChar();
		clearLastKey();
		if (key==key_invincibility) {
			invincible = true;
		}
	}
	
	/**Appears when the player wins the game. 
	 */
	public void paintFrameWin() {
		drawString("You Win!", viewWidth()/2,viewHeight()/3,0,title_font,title_color);
	}
	
	/** Appears when the player hits Esc before the game ends.
	 */
	public void paintFrameEsc() {
		drawString("Game Ended",viewWidth()/2,viewHeight()/3,0,title_font,title_color);
	}
	...
	* title_color */
	...
	public void paintFrameGameOver() {
		...
		if(lives > 0) {
			...
			drawString("Game Over! Try Again!",
				viewWidth()/2,viewHeight()/3,0,title_font,title_color);
		}
	}
